...
func CreateSupplier(c *gin.Context) {
	...
	_, ok := utils.RequireGinPermissions(c, start, requestSummary, "Suppliers", "suppliers.create")
	if !ok {
		// Authorization failed, RequireAdmin already sent error response
		return
	}
	// Decode and parse JSON request body with supplier details
	req, ok := DecodeRequestBody[dtos.Supplier](c, requestSummary, start)
	if !ok {
	...
	if !utils.ValidateGinStructAndRespond(req, c, requestSummary, start, "Suppliers") {
...
func ListSuppliers(c *gin.Context) {
	...
	if _, ok := utils.RequireGinPermissions(c, start, requestSummary, "Suppliers", ""); !ok {
		// Authorization failed, RequireAdmin already sent error response
		return
	}
...
func UpdateSupplier(c *gin.Context) {
	...
	_, ok := utils.RequireGinPermissions(c, start, requestSummary, "Suppliers", "suppliers.update")
	if !ok {
		// Authorization failed, RequireAdmin already sent error response
		return
	}
